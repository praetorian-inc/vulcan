...
DWORD demoShellcodeRtlCreateUserThread(PBYTE pShellcode, SIZE_T szShellcodeLength, DWORD dwProcessId)
{
	...
	LPVOID pRemoteBuffer = VirtualAllocEx(hProcess, NULL, szShellcodeLength, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
	...
	BOOL bStatus = WriteProcessMemory(hProcess, pRemoteBuffer, pShellcode, szShellcodeLength, NULL);
	if (bStatus == 0)
	{
		wprintf(L"[-] Error: Could not write any bytes into the PID (%d) address space.\n", dwProcessId);
		return(1);
	...
	else
	{
		...
		CloseHandle(hProcess);
		VirtualFreeEx(hProcess, pRemoteBuffer, szShellcodeLength, MEM_RELEASE);
		return(0);
	}
