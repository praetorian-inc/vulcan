...
DWORD demoSuspendInjectResume(PCWSTR pszLibFile, DWORD dwProcessId)
{
	...
	if (hThread != NULL)
	{
		...
		printf("[-] Error: Could not open thread\n");
	...
	printf("[*] Change focus to the process you injected into and the DLL should execute.\r\n");
	Sleep(20000);
...
DWORD demoSuspendInjectResume64(PCWSTR pszLibFile, DWORD dwProcessId)
{
	...
	{
		wprintf(L"[-] Error: Could not open process for PID (%d).\n", dwProcessId);
		printf("[-] Error: Could not open process for PID \n");
	...
	if (hThread != NULL)
	{
		...
		wprintf(L"[-] Error: Could not open thread\n");
